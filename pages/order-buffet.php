...
<head>
    ...
    <title>Order Buffet | LAvFood</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>

<body class="bg-amber-50">
    <div class="w-full h-full px-8 py-8">
        <p class="text-stone-800 font-bold text-4xl">Order Buffet</p>
    </div>
    <div class="px-8 space-y-6">
        <form action="/controller/order-buffet-controller.php" method="POST" id="order_buffet_form">
            ...
            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:py-6">
                <label for="amount" class="font-medium text-gray-700 w-32">
                    Number of Pax
                </label>
                <div class="flex-auto">
                    <input id="amount" name="amount" type="number" min=10 class="appearance-none px-3 py-2 w-full border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                </div>
            </div>
...
<script>
    ...
    $('#amount').on('change', function() {
        const qty = $('#amount').val() == "" ? 0 : parseInt($('#amount').val());
        total_price = delivery_price;
        for (let i = 0; i < chosen_menu.length; i++) {
            total_price += (parseInt(chosen_menu[i].foodprice) * qty * 60 / 100);
        }
        $('#total_price').text(`Rp ${total_price}`);
        $('#price').val(total_price);
    })
    ...
    function fetch_chosen_menu() {
        ...
        for (let i = 0; i < chosen_menu.length; i++) {
            ...
            <p class="w-full font-semibold text-green-800">${chosen_menu[i].foodname}</p>
            <button id="remove-${chosen_menu[i].foodid}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            chosen_list.find(`#remove-${chosen_menu[i].foodid}`).click(function() {
                total_price -= (parseInt(chosen_menu[i].foodprice) * qty * 60 / 100);
                $('#total_price').text(`Rp ${total_price}`);
                $('#price').val(total_price);
