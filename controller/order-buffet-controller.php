<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (check_empty($receivedate)) {
        ...
        } else if (check_empty($quantity)) {
            $_SESSION['order_error'] = "Please input the number of pax for the buffet!";
        } else if ((int)$quantity < 10) {
            $_SESSION['order_error'] = "The minimum pax is 10!";
        } else if ($delivery == 0) {
            $_SESSION['order_error'] = "Please choose delivery method!";
        } else if (sizeof($menus) == 0) {
            $_SESSION['order_error'] = "Please choose buffet menu!";
        } else {
            if ($delivery == 1) {
                ...
                $message = "You have been assign to handle a buffet order! Please check your history.";
                $status = "unseen";
            ...
            foreach ($menus as $menu) {
                $menu = (array)$menu;
                $qty = (int)$quantity;
                $query_detail = "INSERT INTO buffettransactiondetail VALUES (?, ?, ?)";
                $detail_statement = $conn->prepare($query_detail);
                $foodid = (int)$menu['foodid'];
                $detail_statement->bind_param("iii", $last_id, $foodid, $qty);
